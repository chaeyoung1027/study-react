...
function Counter(props) {
    ...
    const brokenAdd5 = () => {
        for (let i = 0; i < 5; i++) {
            // 성능상의 이유로 세터 함수로 인한 "변경 사항을 큐에 쌓고 한꺼번에 처리"하므로(=Batch 작업이 이루어지므로), 매 setCount 호출시마다 Counter 함수 호출(re-render)이 이루어지지는 않음
            ...
            // 엄밀히 말하자면 "setCount(0 + 1)" 명령어가 다섯번 큐에 쌓임
            setCount(count + 1)
        }
    }
